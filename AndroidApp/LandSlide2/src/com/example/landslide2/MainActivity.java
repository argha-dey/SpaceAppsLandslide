...
//		valueString = "Limassol,0.12685\nNicosia,1.3181\nPaphos,3.9216\nLimassol,0.12685\nNicosia,1.3181\nPaphos,3.9216\nLimassol,0.12685\nNicosia,1.3181\nPaphos,3.9216";
    	...
    	dropDownLocationList.add(lines[0]);
    	for (int i=1; i<lines.length; i++) {
    		...
    		for (int j=0; j<lines[i].length();j++) {
    			if (Character.isLetter(tmp.charAt(j))) {
    				cities[i] = cities[i] + tmp.charAt(j);
    			}
    			else {
    				values[i] = values[i] + tmp.charAt(j);
    			}
    		}
    		dropDownLocationList.add(cities[i]);
    	}
...
//    	valueString = "Limassol,0.12685\nNicosia,1.3181\nPaphos,3.9216";
    	...
    	for (int i=1; i<lines.length; i++) {
    		tmp = lines[i];
    		int j=0;
    		while (j!=tmp.length() && (Character.isDigit(tmp.charAt(j)) || tmp.charAt(j) == '.')) {
    			j++;
    		}
    		dropDownLocationList.add(tmp.substring(j));
    		values[i-1] = tmp.substring(0,j);
    	}
    	
    	String selection = dropDownLocations.getSelectedItem().toString();
    	for (int i=0; i<dropDownLocations.getCount(); i++) {
    		if (dropDownLocations.getItemAtPosition(i).equals(selection)) {
    			checkValue = Double.parseDouble(values[i]);
    			break;
    		}
    	}
